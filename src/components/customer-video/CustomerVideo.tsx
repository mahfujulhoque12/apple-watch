...
const CustomerVideo: React.FC = () => {
  ...
  return (
    <section className="mt-5">
      <MaxWidthWrapper>
        <h2 className="text-gray-200 text-2xl font-bold text-center pb-5">
            ...
            <CarouselContent className="gap-5">
              {videoData.map(({ url, thumbnail, title }, index) => {
                ...
                return (
                  <CarouselItem
                    key={index}
                    className="p-4 relative shadow-md z-10 border border-gray-700 rounded-lg hover:shadow-lg transition-shadow duration-300 mb-2 cursor-pointer"
            ...
            {dotIndices.map((targetIndex, i) => (
              <button
                ...
                className={`
                  ...
                  ${activeDot === i ? "bg-[#183B4E] scale-125" : "bg-gray-300"}
                `}
        ...
        <div className="mt-5">
          <Link
            href="#order"
            className="relative overflow-hidden text-xl font-bold text-white bg-[#183B4E] px-6 py-3 rounded-md w-full block text-center group"
          >
